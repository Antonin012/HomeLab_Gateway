...
- Vérification de l'IP virtuelle : Mon appareil a bien reçu l'IP 10.X.X.X.

- Accès au Home Lab : Un ping vers l'IP locale du serveur Nextcloud (192.168.1.X) répond avec succès.
...
- Alternative (LAN) : Si vous souhaitez que vos machines locales (hors VPN) utilisent aussi le filtrage, configurez leur DNS sur l'IP locale de votre Debian (ex: `192.168.1.X`).
